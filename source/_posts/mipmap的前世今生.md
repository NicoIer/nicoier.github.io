...
我习惯把 Mipmap 理解为：**预先算好的平均结果**。

这句话省略了不少细节，但抓住了它真正解决的问题。纹理缩小时，一个屏幕像素往往覆盖纹理中的一片区域。直接在中心 UV 取一个值，回答的是“这个点是什么颜色”；渲染真正需要的却是“这片区域整体是什么颜色”。如果每帧都临时计算区域平均，代价太高，于是我们把不同尺度的过滤结果提前算好，运行时只选择合适的一层。

沿着这条思路，Mipmap 的生成、LOD、Trilinear Filtering 和各向异性过滤其实是一件事的不同部分：
...
根据当前 footprint 选择尺度
        ↓
footprint 过于细长时，再沿长轴补充采样
```

# 从一个 UV 到 Pixel Footprint

这里不再重新介绍 Pixel、Texel 和 UV，只保留一个后面必须用到的区别：片元着色器收到的是一个确定的 UV，屏幕像素在纹理空间中覆盖的却是一片区域。
...
这里我真正关心的不是物体离相机多远，而是 **UV 在相邻屏幕像素之间变化得有多快**。距离、FOV、分辨率、模型缩放、表面朝向和 UV Tiling 最终都会反映到这个变化率里。
...
对于 Box Filter，Level $l$ 中的一个 texel 对应 Level 0 中约 $2^l\times2^l$ 范围的平均结果。实际的离线生成器也可以使用覆盖范围更大、频率响应更好的滤波核，因此更准确的说法是“预先算好的低通结果”；在这篇文章的推导里，我仍然把它简称为平均结果。
...
1. 新建一个 URP 项目，把 Shader 保存为 `.shader`，再创建对应材质。
2. 准备一张高频棋盘格纹理，启用 `Generate Mip Maps`，先把 Filter Mode 设为 Trilinear。
3. 把材质赋给一个拉长的 Plane，将纹理 Tiling 调到 32 或更高。
4. 把相机压低，以接近贴地的角度观察 Plane。
5. 将 `Mip Color Strength` 设为 1，可以看到 LOD 色带；调回 0，则看到真实纹理采样结果。
6. 移动相机、改变分辨率、修改 Tiling 或替换不同尺寸的纹理，色带边界都会变化。这正好说明 LOD 不是一个单独的距离函数。
7. 最后对比纹理的 Aniso Level 为 0、4、8、16 时的斜视结果，并确认 Quality Settings 中没有全局禁用 Anisotropic Textures。
...
写到这里，我想到另一个问题：既然摩尔纹来自采样不足，而 TAA 会在不同帧改变亚像素采样位置，它能不能缓解纹理摩尔纹？
...
这条链路里，真正把所有概念串起来的不是距离，也不是某个固定的过滤选项，而是 pixel footprint。
